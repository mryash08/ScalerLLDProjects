...
public class Game {
    ...
    public void undo() throws EmptyMovesException {
        ...
        if(!moves.isEmpty()){
           Move moveundo =  moves.remove(moves.size()-1);
            nextPlayerMoveIndex -= 1;
            nextPlayerMoveIndex %= players.size();
            winningStrategy.undo(board, moveundo);
            int row = moveundo.getCell().getRow();
            int col = moveundo.getCell().getCol();
        ...
        } else throw new EmptyMovesException("Undo operation can't be performed as the moves list is empty");
    }
}
